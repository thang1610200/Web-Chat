...
// 		var id = $(this).attr("id_video_call");
// 		var save = "#call-wraper" + id;
...
//--- heart like and unlike 
		$('.heart').click(function(){
		  if($(this).attr("class") == "likes heart"){
			$(this).addClass('happy').removeClass('broken');
			var data = {
				id_post: $(this).attr("post_id"),
				id_user: $('#id_user').val()
			}
			var base_url = location.protocol + "//" + document.domain + ":" + location.port;
            $.ajax({
                url: base_url + "/home/likeajax",
                type: "POST",
                data: data,
                dataType: "json",
		  ...
		  else if($(this).attr("class") == "likes happy heart") {
			...
			var base_url = location.protocol + "//" + document.domain + ":" + location.port;
            $.ajax({
                url: base_url + "/home/deletelikeajax",
                type: "DELETE",
                data: data,
                dataType: "json",
                success: function(res){
                    if(res && res.status_code == 200){
                        //document.getElementById('example').innerHTML = String(kq);
						location.reload();
						//$("#loaddiv").load(location.href + " #loaddiv>*","");
                    }
                }
		...
		$('.add-friend').click(function(){
			var data = {
				id_user:$('#id_user_f').val(),
				id_friend : $(this).attr("id_friend")
			}
			var base_url = location.protocol + "//" + document.domain + ":" + location.port;
			$.ajax({
		...
		$('.confirmf').click(function(){
			var data = {
				id_user_f:$('#idofreceiver').val(),
				id_friend_list:$(this).attr("idsender")
			}
			var base_url = location.protocol + "//" + document.domain + ":" + location.port;
			$.ajax({
				url: base_url + "/home/confirm",
				type: "POST",
				data:data,
				dataType: "JSON",
				success: function(res){
...
//--- side message box	
	// $('.friendz-list > li,.drops-menu > li > a.show-mesg').on('click', function() {
	// 	$('.chat-box').addClass("show");//.trigger("change-class");
	// 	return false;
	// });	
...
jQuery(".post-comt-box textarea").on("keydown", function(event) {
	if (event.keyCode == 13) { // nhấn phím Enter để Đăng Comment
		var data = {
			comment: jQuery(this).val(),
			post_id: $(this).attr("postid"),
			id_user: $('#id_comment').val()
		}
		var base_url = location.protocol + "//" + document.domain + ":" + location.port;
		$.ajax({
			...
			success: function(res){
				if(res && res.status_code == 200){
					//$('#page-contents').load(location.href + " #page-contents> *","");
					location.reload();
				}
			}
		});
